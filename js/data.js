...
const TIP_DATA = {
  ...
  /* ── Feed Items (populated via manual Claude-cowork fetch sessions + admin manual entries) ── */
  feedItems: [],
...
const DataManager = {
  ...
  init() {
    ...
    this.mergeAutoFetchedSnowbit();
    this.save();
  },

  // Validates one TIP_AUTOFEED entry before it's allowed anywhere near
  // feedItems. autofeed.js is hand-written via an interactive Claude
  // session (WebSearch) with output derived from arbitrary web pages —
  // this is the actual trust boundary in the shipped app, so re-check
  // everything here even though lib/intel/schemas.mjs's sanitizers are
  // meant to be applied during research too. Returns null (drop) on any
  // failure rather than trying to repair a bad field.
  ...
  // Merges TIP_AUTOFEED (js/autofeed.js, hand-updated via Claude cowork
  // sessions) directly into the live, published feed — same as
  ...
  mergeAutoFetchedSnowbit() {
    if (typeof TIP_AUTOFEED_SNOWBIT === 'undefined' || !Array.isArray(TIP_AUTOFEED_SNOWBIT) || !TIP_AUTOFEED_SNOWBIT.length) return;
    const knownIds = new Set(TIP_DATA.snowbitAdvisories.map(s => s.id));
    TIP_AUTOFEED_SNOWBIT
      .filter(item => item && item.id && !knownIds.has(item.id) && item.title && item.severity && item.summary)
      .forEach(item => TIP_DATA.snowbitAdvisories.unshift(Object.assign(
        { tags: [], execSummary: [], howItWorks: [], mfaGaps: [], timeline: [], affectedComponents: [], iocs: [], dataprimeQueries: [], remediation: [], snowbitResponse: [] },
        item)));
  },
